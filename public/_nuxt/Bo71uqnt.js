import{u as $}from"./bwIM7N4K.js";import{x as z,r as c}from"./BeZXCPwx.js";const p=c(!1),u=c([]),d=c("");typeof window<"u"&&(d.value=localStorage.getItem("selected_printer")||"");const k=()=>{const f=$(),b=async()=>{typeof window<"u"&&window.electronAPI?(p.value=!0,await g()):(console.warn("Electron API not found. Printing will use browser default."),p.value=!1)},g=async()=>{if(typeof window<"u"&&window.electronAPI)try{const t=await window.electronAPI.getPrinters();if(u.value=t.map(e=>e.name),!d.value){const e=t.find(i=>i.isDefault);e&&m(e.name)}}catch(t){console.error("Failed to fetch printers:",t)}},m=t=>{d.value=t,localStorage.setItem("selected_printer",t)},v=(t,e={})=>{const i=t?.items?.map((l,w)=>`
    ...
    `;typeof window<"u"&&window.electronAPI?(window.electronAPI.printHTML({html:t,printerName:d.value}),f.addToast("Тестовая страница отправлена","success")):x(t)},generateReceiptHtml:h,generateInvoiceHtml:v}};export{k as u};
