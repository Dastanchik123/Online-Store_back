import{u as $}from"./yTi2mtLW.js";import{u as z}from"./CPI8uL9m.js";import{r as c}from"./Dq9divzr.js";const p=c(!1),u=c([]),d=c("");typeof window<"u"&&(d.value=localStorage.getItem("selected_printer")||"");const A=()=>{const f=$(),b=async()=>{typeof window<"u"&&window.electronAPI?(p.value=!0,await g()):(console.warn("Electron API not found. Printing will use browser default."),p.value=!1)},g=async()=>{if(typeof window<"u"&&window.electronAPI)try{const t=await window.electronAPI.getPrinters();if(u.value=t.map(e=>e.name),!d.value){const e=t.find(i=>i.isDefault);e&&m(e.name)}}catch(t){console.error("Failed to fetch printers:",t)}},m=t=>{d.value=t,localStorage.setItem("selected_printer",t)},v=(t,e={})=>{const i=t?.items?.map((l,w)=>`
      <tr style="border-bottom: 1px solid #f1f5f9;">
        <td style="padding: 10px 8px; color: #64748b; font-size: 11px;">${w+1}</td>
        <td style="padding: 10px 8px; font-weight: 500; color: #1e293b; font-size: 12px;">${l.product_name||l.name}</td>
        <td style="padding: 10px 8px; text-align: center; color: #64748b; font-size: 12px;">${l.quantity} шт.</td>
        <td style="padding: 10px 8px; text-align: right; color: #64748b; font-size: 12px;">${Math.round(l.price).toLocaleString()}</td>
        <td style="padding: 10px 8px; text-align: right; font-weight: 600; color: #0f172a; font-size: 12px;">${Math.round(l.price*l.quantity).toLocaleString()}</td>
      </tr>
    `).join("")||'<tr><td colspan="5" style="text-align:center; padding: 20px;">Нет данных</td></tr>',n=e.receipt_title||e.site_name||"BRAND STORE",o=new Date(t?.created_at||new Date).toLocaleString("ru-RU",{day:"2-digit",month:"long",year:"numeric",hour:"2-digit",minute:"2-digit"}),a=t?.order_number||t?.id||"_______",s=Number(t?.total_amount||t?.total||0),r=Number(t?.discount||0),y=Number(t?.total||s);return`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
        ...
        <div class="invoice-box">
          ...
          <div class="info-grid">
            ...
            <div>
              ...
              <div style="font-weight: 600;">${t?.user?.name||"Розничный покупатель"}</div>
              ${t?.user?.id&&!String(t.user.id).includes("retail")?`<div style="color: #64748b;">ID: ${t.user.id}</div>`:""}
            </div>
          </div>
          ...
          <div class="total-box">
            ...
            ${r>0?`<div class="total-row" style="color: #ef4444;"><span>Скидка:</span><span>-${r.toLocaleString()}</span></div>`:""}
            <div class="total-row grand-total">
              <span>ИТОГО:</span>
              <span>${Math.round(y).toLocaleString()} сом</span>
            </div>
          </div>
    ...
    `},h=(t,e={})=>{const i=t.items.map(s=>`
      <tr>
        <td style="padding: 1mm 0; font-size: 10px;">${s.product_name||s.name}</td>
    ...
    `).join(""),n=e.receipt_title||e.site_name||"МОЙ МАГАЗИН",o=new Date().toLocaleString("ru-RU"),a=t.order_number||"OFF-LINE";return`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
    ...
    `},x=t=>{const e=document.createElement("iframe");e.style.position="fixed",e.style.width="0",e.style.height="0",e.style.border="0",e.style.visibility="hidden",document.body.appendChild(e);let i=!1;const n=()=>{i||(i=!0,e.parentNode&&document.body.removeChild(e))},o=e.contentWindow,a=o?.document;if(!o||!a){n();return}a.open(),a.write(t),a.close(),setTimeout(()=>{o.onafterprint=n,o.focus(),o.print(),setTimeout(n,1e4)},300)};return{isConnected:p,printers:u,activePrinter:d,initPrinter:b,fetchPrinters:g,setPrinter:m,printReceipt:async(t,e="thermal")=>{try{let i="";if(typeof t=="object"&&t!==null&&(!t.items||!Array.isArray(t.items))){console.warn("Order data has no items, falling back to server-side printing");const n=t.uuid||t.id;n&&(t=n)}if(typeof t=="object"&&t!==null)i=h(t);else{const{getAuthToken:n,baseURL:o}=z(),a=n(),s=e==="thermal"?`/reports/order/${t}/thermal/html`:`/reports/order/${t}/html`,r=await fetch(`${o}${s}`,{headers:{Authorization:`Bearer ${a}`}});if(!r.ok)throw new Error("Ошибка сервера при получении чека");i=await r.text()}typeof window<"u"&&window.electronAPI?window.electronAPI.printHTML({html:i,printerName:d.value}):x(i)}catch(i){console.error("Printing failing:",i),f.addToast("Ошибка печати: "+i.message,"error")}},testPrint:async()=>{const t=`
      <div style="font-family: sans-serif; padding: 20px; border: 2px solid black; text-align: center;">
        ...
        <p>Принтер: ${d.value||"По умолчанию"}</p>
        <hr>
    ...
    `;typeof window<"u"&&window.electronAPI?(window.electronAPI.printHTML({html:t,printerName:d.value}),f.addToast("Тестовая страница отправлена","success")):x(t)},generateReceiptHtml:h,generateInvoiceHtml:v}};export{A as u};
